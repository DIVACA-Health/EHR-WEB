            ...
            >
              <div className="flex  justify-between items-center min-h-[10%] pl-6 pr-6 border-b-[1px] mb-2 border-gray-200 shadow-sm">
                <div className='flex flex-col'>
                    <h2>Date of Visit : {selectedPrescription ? formatDate(selectedPrescription.prescribedDate) : ''}</h2>
                    <h2>Doctor : {selectedPrescription && selectedPrescription.doctor ? `Dr ${selectedPrescription.doctor.firstName} ${selectedPrescription.doctor.lastName}` : ''}</h2>
                </div>
                      ...
                      {isOpen && (
                        <div className="divide-y text-sm pl-4 pr-4 pt-2 pb-2 rounded-b-[12px]">
                          <div className="flex justify-between px-4 py-3">
                            <span>Diagnosis</span>
                            <span className="text-gray-700 font-medium">{selectedPrescription.diagnosis}</span>
                          </div>
                          <div className="flex justify-between px-4 py-3">
                            <span>Instructions</span>
                            <span className="text-gray-700 font-medium">
                              {selectedPrescription.medications && selectedPrescription.medications.length > 0
                                ? selectedPrescription.medications[0].instructions
                                : ''}
                            </span>
                          </div>
                          <div className="flex justify-between px-4 py-3">
                            ...
                            <span className="text-gray-700 font-medium">
                              {selectedPrescription.medications && selectedPrescription.medications.length > 0
                                ? `${selectedPrescription.medications[0].medication} (${selectedPrescription.medications[0].dosage})`
                                : ''}
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="w-full border-[0.8px] border-[rgba(235,235,235,1)] rounded-[8px] shadow-sm">
                      ...
                      {isOpen1 && (
                        <div className="divide-y text-sm pl-4 pr-4 pt-2 pb-2 rounded-b-[8px]">
                          <div className="flex justify-between px-4 py-3">
                            <span>Notes</span>
                            <span className="text-gray-700 font-medium">{selectedPrescription.notes || 'No notes'}</span>
                          </div>
                          <div className="flex justify-between px-4 py-3">
                            <span>Status</span>
                            <span className="text-gray-700 font-medium">{selectedPrescription.status}</span>
                          </div>
                        </div>
                      )}
                    </div>
